...
float sgemm_error_tensor_core(const unsigned M, const unsigned K, const unsigned N, const float *a, const float *b,
                              const float *ret) {
    float error = 0.0f;
    for (unsigned i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            float value = 0.0f;
            for (int k = 0; k < K; ++k) {
                value += __half2float(half(a[i * K + k]) * half(b[k * N + j]));
            }
            error = fmaxf(error, fabsf(value - ret[i * N + j]));
        }
    }
    return error;
}
...
int main() {
    ...
    call_sgemm_tensor_core_v0(M, K, N, a, b, ret);
    std::cout << "call sgemm tensor core v0 kernel: " << sgemm_error_tensor_core(M, K, N, a, b, ret) << std::endl;
    // call sgemm tensor core v1 kernel
    call_sgemm_tensor_core_v1(M, K, N, a, b, ret);
    std::cout << "call sgemm tensor core v1 kernel: " << sgemm_error_tensor_core(M, K, N, a, b, ret) << std::endl;
    // host free
    free(a);
    free(b);
    free(ret);
