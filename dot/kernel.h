...
__global__ void dot_warp_shuffle_down(const unsigned N, const float *a, const float *b, float *ret) {
    ...
    __syncwarp();
    shuffle_down_reduce<float, AddFunctor<float>, WARP_SIZE>(value);
    if (laneIdx == 0) {
        tmp[warpIdx] = value;
    }
    __syncthreads();
    if (warpIdx == 0) {
        ...
        shuffle_down_reduce<float, AddFunctor<float>, WARP_NUM>(value);
        if (laneIdx == 0) {
            atomicAdd(ret, value);
        }
    }
...
__global__ void dot_warp_shuffle_xor_v0(const unsigned N, float *a, float *b, float *ret) {
    ...
    __syncwarp();
    shuffle_xor_reduce<float, AddFunctor<float>, WARP_SIZE>(value);
    if (laneIdx == 0) {
        tmp[warpIdx] = value;
    }
    __syncthreads();
    if (warpIdx == 0) {
        value = laneIdx < WARP_NUM ? tmp[laneIdx] : 0.0f;
        shuffle_xor_reduce<float, AddFunctor<float>, WARP_NUM>(value);
        if (laneIdx == 0) {
            atomicAdd(ret, value);
        }
    }
...
__global__ void dot_warp_shuffle_xor_v1(const unsigned N, float *a, float *b, float *c) {
    ...
    __syncwarp();
    shuffle_xor_reduce<float, AddFunctor<float>,WARP_SIZE>(sum);
    if (lane == 0) {
        smem[warpIdx] = sum;
    }
    __syncthreads();
    sum = lane < NUM_WARP ? smem[lane] : 0.0f;
    shuffle_xor_reduce<float, AddFunctor<float>, NUM_WARP>(sum);
