...
前两天看了几个sgemm的博客（[1](https://zhuanlan.zhihu.com/p/518857175)、[2](https://zhuanlan.zhihu.com/p/410278370)），虽然都没完全看懂，不过意识到几个问题，主要还是坐标映射布局上面，所以今天写了个v3版本。

一：读全局内存，直接用线程的块内idx求对应tile里的行列，不管thread tile尺寸了，相当于直接把线程block当一维求对应元素位置。

二：按float4读，因为正好是尺寸特殊所以简单就写一个线程分别读a和b的一个float4。

三：thread tile对应的结果矩阵不是连续布局，而是类似strip的跨步的分散布局。

比如连续内存是某线程管\[0\]\[0\]、\[0\]\[1\]、\[1\]\[0\]、\[1\]\[1\]

而分散布局是，假如blockDim是2*2，那么线程的thread tile对应\[0\]\[0\]、\[0\]\[2\]、\[2\]\[0\]、\[2\]\[2\]。

四：不关注bank conflict了，首先读全局内存写共享内存，因为是把block线程当一维处理，自然不会有bank conflict，而读共享内存到寄存器，虽然有点但是也没管了。

五：tile尺寸改为128\*8、8\*128，thread tile是8\*8，等于数据复用度提高了很多。

测试结果的话，第一次虽然有所提升，但是仍然达不到cublas的50%，看了下ncu报告，居然有优化建议，说是grid总数至少是多处理器数目2倍，否则有的多处理器空闲会浪费，正好我的输入数据计算出来的grid总数正好是sm的一倍。

简单考虑直接把输入数据一个维度提高一倍，最后测试出来结果大概是cublas的2/3，也算是比较满意了，

## 2025/11/20
看nvidia官方博客，发现可以异步把全局内存搬运到共享内存，试了下，然而是负优化。